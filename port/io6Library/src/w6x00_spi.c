...
void wizchip_spi_initialize(void)
{
    ...
    // spi_init(SPI_PORT, 5000 * 1000); //< 5MHz is too fast for the EEPROM this shares SPI lines with. 
    spi_init(SPI_PORT, 1000 * 1000); //< SPI0 @ 1MHz
...
void wizchip_initialize(void)
{
    ...
    volatile uint32_t phy_retry_count = 0; //! Counts how many times we have tried to get a link to ethernet
    ...
    do
    {
        ...
        phy_retry_count++;
        // if(phy_retry_count > 1000000) { //! @note 1000000 was determined by increasing the magnitude until the delay was long enough to allow the PHY to link
        // if(phy_retry_count > 500000) {
        // if(phy_retry_count > 250000) { // ~10s
        // if(phy_retry_count > 100000) { // ~4s
        if(phy_retry_count > 200000) { // ~8s
            #ifdef BSERIES_EN
            phy_linked = false; // redundant, but just in case
            #endif
            break; //! breaks out of the loop if no link is found
        }
...
} // Formatted print of IPv6 addresses
